...
void st7920_init(st7920_t *d, swspi_t *spi, swgpio_t *rs, void *pvFontDef) {
    ...
    d->d.parent = d;
    d->d.curX=0; d->d.curY=0; //d->CurrentX = 0; d->Current`Y = 0;
    ...
    d->d.widthScale = 3;
    d->d.posmask = 7;
    d->d.dwFrameByteSize = (d->d.frameWidth * d->d.frameHeight) / 8;
    d->d.invposmask = 7; //position from 7-0 to 0-7
    d->d.oneLineOffsetSize = d->d.frameWidth >> 3;
    d->d.pFrameBuf = d->buf;
    if(pvFontDef) d->d.pFont = (struct FontDef*)pvFontDef;
    d->d.fill_color = &st7920_hal_fill;
    d->d.update = &st7920_hal_update;
    d->d.pixeldraw = &st7920_hal_pixeldraw;
	delay_ms(40);
...
void st7920_hal_fill(lcddev_t *d, uint8_t color) { st7920_fill((st7920_t*)d->parent, color); }
...
void st7920_hal_update(lcddev_t *d) { st7920_update((st7920_t*)d->parent); }
...
void st7920_hal_pixeldraw(fontdraw_t *d, uint32_t x, uint32_t y, int color) { st7920_DrawPixel((st7920_t*)d->parent, x, y, color); }
#endif
// 1-31 => 95-125
// 32-126 => 0-94
// 127-255 => 126-254
...
void st7920_DrawPixel(st7920_t *d, uint8_t x, uint8_t y, uint8_t color) {
    st7920_hal_pixeldraw(&d->d, x, y, color);
}
#endif
